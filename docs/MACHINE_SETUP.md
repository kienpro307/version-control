# MVM Machine Setup (Run Once Per Machine)

> Chạy 1 lần trên mỗi máy mới để enable `/mvm` auto-run

---

## Step 1: Setup GEMINI.md Global Config

Tạo file `~/.gemini/GEMINI.md` với nội dung:
...
## Step 3: Add MCP Config to Antigravity

Open Antigravity Settings → MCP Servers → Add:
...
Restart Antigravity, then type `/mvm` → should auto-run without Accept button.
...
**Windows:**
```powershell
# Create GEMINI.md directory
mkdir "$env:USERPROFILE\.gemini" -Force

# Copy template (from this repo)
copy "docs\GEMINI_TEMPLATE.md" "$env:USERPROFILE\.gemini\GEMINI.md"
```

**macOS/Linux:**
```bash
mkdir -p ~/.gemini
cp docs/GEMINI_TEMPLATE.md ~/.gemini/GEMINI.md
```
