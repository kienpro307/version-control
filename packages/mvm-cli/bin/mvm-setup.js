...
// MVM Setup CLI
// Usage: npx @kien307/mvm-cli <project-path> <project-uuid>
// Or after install: mvm-setup <project-path> <project-uuid>
...
╠═══════════════════════════════════════════════════════════════╣
║                                                               ║
║  Usage: npx @kien307/mvm-cli <path> <uuid>          ║
║                                                               ║
║  Example:                                                     ║
║  npx @kien307/mvm-cli D:\\Projects\\MyApp abc-123    ║
║                                                               ║
║  Get UUID from: MVM UI → Create/Select project → Copy ID      ║
║  MVM UI: https://mvm.vercel.app (or your deployment)          ║
╚═══════════════════════════════════════════════════════════════╝
`;

if (!targetPath || !projectUuid || targetPath === '--help' || targetPath === '-h') {
    console.log(HELP_TEXT);
    process.exit(targetPath === '--help' || targetPath === '-h' ? 0 : 1);
}
...
║                                                               ║
║  Next: Ensure GEMINI.md is configured on this machine.        ║
║  See: docs/MACHINE_SETUP.md for global config.                ║
╚═══════════════════════════════════════════════════════════════╝
`);
